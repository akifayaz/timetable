...
const [goal, setGoal] = useLocal<number>("oaa_next_goal", 300); // dakika
  ...
  const notificationLogDateRef = useRef<string>(dateKey(new Date()));
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      if (notificationLogDateRef.current !== todayKeyTick) {
        notificationLogRef.current.clear();
        notificationLogDateRef.current = todayKeyTick;
        lastMinuteRef.current = null;
      }

      const previousMinute = lastMinuteRef.current;
      const windowStart = previousMinute === null ? mins : previousMinute + 1;
      ...
      classes.forEach((cls) => {
        ...
        const occursNow = (targetMin: number) => targetMin >= windowStart && targetMin <= mins;

        if (occursNow(startMin)) {
          notify("start");
        }
        if (occursNow(endMin)) {
          notify("end");
